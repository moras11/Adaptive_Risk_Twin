...
fig = plt.figure(figsize=(8, 5))
sns.boxplot(
    data=df[df["amount"] < df["amount"].quantile(0.99)], x="isFraud", y="amount"
)
...
sns.boxplot(data=origin_melt, x="balance_type", y="balance_value", hue="isFraud")
plt.yscale("log")
...
fig = plt.figure(figsize=(10, 5))
sns.boxplot(data=dest_melt, x="balance_type", y="balance_value", hue="isFraud")
plt.yscale("log")
...
fig = plt.figure(figsize=(10, 5))
sns.lineplot(
    data=df.groupby("step")["isFraud"].mean().reset_index(), x="step", y="isFraud"
)
